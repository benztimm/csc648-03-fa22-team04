    ...
    try:
        ...
        if _email and _password and request.method == 'POST':
            query = """
                ...
                """
            data = _email
            auth_email = db.execute_query(query, data)
            if auth_email and _password == decode(_password):
                session['loggedin'] = True
                session['id'] = auth_email['id']
                session['username'] = auth_email['username']
...
def decode(user_password):
    ...
    WHERE
        ...
        """
    results = db.execute_query(query, user_password)
